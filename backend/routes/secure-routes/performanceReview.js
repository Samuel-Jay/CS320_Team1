  ...
  try {
    ...
    User.findOne({ email: req.body.email }, async (err, user) => {
      ...
      if (!allowed.includes(req.body.reviewerId)) {
        ...
        !User.find(
          { managerId: user.employeeId }.includes(req.body.revieweeId) &&
            req.body.revieweeId == user.employeeId
        )
      ) {
        return res.json({
          message:
            "Cannot request review from employee who isn't peer or subordinate.",
        });
